...
resource "aws_instance" "web_aws" {
  ...
  user_data = templatefile("${path.module}/templates/web.tpl", { dc = "aws", use_proxy = false, consul_cluster_addr = "", shared_services_private_ip = aws_instance.shared_services.private_ip })
